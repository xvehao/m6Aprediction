...
logit.pre.auc=list()
logit.pre=matrix(0,nrow=test_size,ncol=100)
for (i in 1:num_y) {
...
SVM.pre.auc=list()
SVM.pre=matrix(0,nrow=(test_size),ncol=100)
for (i in 1:num_y) {
...
rf.pre.auc=list()
rf.pre=matrix(0,nrow=(test_size),ncol=100)
for (i in 1:num_y) {
...
### find the sites selected by ENLR
selected_sites_logit=matrix(0,nrow = 100,ncol=ncol(X))
colnames(selected_sites_logit)=colnames(X)
for (i in 1:100){selected_sites_logit[i,predictors(logit[[i]])]=1}
aggre_sites_logit=rbind(mouse_RPKM_input[idx,1],colSums(selected_sites_logit))
colnames(aggre_sites_logit)=mouse_RPKM_input[idx,1]
aggre_sites_logit[,order(-aggre_sites_logit[2,])]
